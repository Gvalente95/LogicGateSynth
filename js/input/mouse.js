...
_canvas.addEventListener("wheel", (e) => {
  e.preventDefault();
  if (_input.keys["shift"] || _input.keys["meta"] || _input.keys["alt"]) setScale(clamp(_scale + -e.deltaY * 0.005, 0.4, 2));
  else _camera.move(e.deltaX, e.deltaY);
});

window.addEventListener("contextmenu", (e) => {
  e.preventDefault();
  if (!_hovElement && !_selBox.nodes.length) _selBox.reset();
...
window.addEventListener("mousedown", (e) => {
  ...
  if (_selBox.nodes && _input.keys["alt"]) _selBox.dupplicateNodes([-20, -20]);
  else if (_hovElement) {
    if (!_selBox.contains(_hovElement)) _selBox.reset();
  ...
  if (!_selElement && !_selHandle && !_menu.contextMenuActive && e.button === 0) _selBox.init();
  else if ((_input.keys["shift"] || _input.keys["meta"]) && _hovElement && e.button === 0) {
    if (!_selBox.tryClearNode(_hovElement)) _selBox.tryPush(_hovElement);
  }
  _mouse.pressed = true;
    ...
    if (hov) {
      ...
      if (!_input.keys["shift"]) {
        if (hov.type === "BOOL") {
          ...
          for (const n of _selBox.nodes) if (n.type === "BOOL") n.setOutput(n.output ? 0 : 1);
        } else if (hov.onInspect !== undefined) hov.onInspect();
        else if (_mouse.dbClicked && hov) _menu.toggleContextMenu(_mouse.pos, hov);
      }
    } else if (_menu.contextMenuActive) _menu.toggleContextMenu(false);
    // else _menu.toggleContextMenu(_mouse.pos);
  }
  _selBox.active = false;
  if (_selHandle && !wasClick) {
    var other = _hovHandle;
    if (!_hovHandle || !_selHandle.tryAttachTo(_hovHandle)) other = Handle.get(_mouse.pos, _selHandle);
...
function dragSelOutput() {
  ...
  if (n.isFloat) {
    ...
    const speed = (Math.min(n.maxValue, 100) - Math.max(n.minValue, -100)) * 0.005;
...
function setHoverElements() {
  ...
  _renameHov = null;
  if (_hovElement && _menu.contextMenuActive && _mouse.pressed) _menu.toggleContextMenu(null);
  if (!_selBox.active && _mouse.pressed && !_selHandle) _selBox.moveNodes();
  if (_selHandle) {
    markObstaclesDirty();
    const other = Handle.get(_mouse.pos, _selHandle, false);
    if (!other || !_selHandle.canAttachTo(other)) _selHandle.place(_selHandle.start, _mouse.world);
    else _selHandle.place(_selHandle.start, other.start);
  }
  if (_renameNode && _renameProperty === "output" && _mouse.pressed) dragSelOutput();
  else if (_selElement) {
